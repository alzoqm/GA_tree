...
void check_tensors(
    ...
    const torch::Tensor& next_indices, // [수정] next_indices 텐서 체크 추가
    ...
    TORCH_CHECK(next_indices.device().is_cuda(), "Next_indices tensor must be on a CUDA device"); // [수정] 추가
    ...
    TORCH_CHECK(next_indices.scalar_type() == torch::kInt32, "Next_indices tensor must be of type int32"); // [수정] 추가
    ...
    TORCH_CHECK(next_indices.is_contiguous(), "Next_indices tensor must be contiguous"); // [수정] 추가
    ...
    TORCH_CHECK(next_indices.dim() == 1, "Next_indices tensor must be 1D"); // [수정] 추가
    ...
    TORCH_CHECK(next_indices.size(0) == pop_size, "Next_indices tensor pop_size mismatch"); // [수정] 추가
    ...
    TORCH_CHECK(results.size(1) == 3, "Results tensor must have 3 columns");
}
...
void predict_cuda(
    ...
    torch::Tensor next_indices_tensor, // [수정] 파라미터 추가
    ...
    launch_predict_kernel(
        ...
        next_indices_tensor.data_ptr<int>(), // [수정] 인자 전달
