...
void check_tensors(
    ...
    const torch::Tensor& next_indices, // [추가] next_indices 텐서 체크
    ...
    TORCH_CHECK(next_indices.device().is_cuda(), "Next_indices tensor must be on a CUDA device"); // [추가]
    ...
    TORCH_CHECK(next_indices.scalar_type() == torch::kInt32, "Next_indices tensor must be of type int32"); // [추가]
    ...
    TORCH_CHECK(next_indices.is_contiguous(), "Next_indices tensor must be contiguous"); // [추가]
    ...
    TORCH_CHECK(next_indices.dim() == 1, "Next_indices tensor must be 1D"); // [추가]
    ...
    TORCH_CHECK(next_indices.size(0) == pop_size, "Next_indices tensor pop_size mismatch"); // [추가]
...
void predict_cuda(
    ...
    torch::Tensor next_indices_tensor, // [추가]
    ...
    launch_predict_kernel(
        ...
        next_indices_tensor.data_ptr<int>(), // [추가]
