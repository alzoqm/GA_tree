...
config = {
    ...
    'feature_comparison_map': FEATURE_COMPARISON_MAP, # [수정] feature_pair -> feature_comparison_map
    'feature_bool': FEATURE_BOOL,
    'all_features': ALL_FEATURES, # [수정] 임포트한 ALL_FEATURES 사용
}
...
def run_mutation_test(name: str, mutation_op: BaseMutation, population: GATreePop, if_save_model=False):
    ...
    # [수정] 원본 GATree 객체 생성 (시각화용) - feature_comparison_map 인자 사용
    tree_before = GATree(MAX_NODES, MAX_DEPTH, MAX_CHILDREN, FEATURE_NUM, FEATURE_COMPARISON_MAP, FEATURE_BOOL,
    ...
    # 3. 변이 후 결과 확인
    # [수정] 변이 후 GATree 객체 생성 (시각화용) - feature_comparison_map 인자 사용
    tree_after = GATree(MAX_NODES, MAX_DEPTH, MAX_CHILDREN, FEATURE_NUM, FEATURE_COMPARISON_MAP, FEATURE_BOOL,
    ...
    if not torch.equal(original_tensor[0], mutated_tensor[0]): # [수정] 원본 텐서와 비교
...
if __name__ == '__main__':
    ...
    # [수정] GATreePop 생성자에 feature_comparison_map 인자 추가
    initial_population = GATreePop(POP_SIZE, MAX_NODES, MAX_DEPTH, MAX_CHILDREN, FEATURE_NUM, FEATURE_COMPARISON_MAP, FEATURE_BOOL)
